...
import static uk.gov.hmcts.sptribs.testutil.TestConstants.POSTPONE_HEARING_NOTIFY_PARTIES_MID_EVENT_URL;
...
public class PostponeHearingNotifyPartiesIT extends IntegrationTestBase {
    ...
    void shouldNotReturnErrorsOnMidEvent() throws Exception {
        ...
        String response = mockMvc.perform(post(POSTPONE_HEARING_NOTIFY_PARTIES_MID_EVENT_URL)
    ...
    void shouldReturnErrorsOnMidEvent() throws Exception {
        mockMvc.perform(post(POSTPONE_HEARING_NOTIFY_PARTIES_MID_EVENT_URL)
            .contentType(APPLICATION_JSON)
            .header(SERVICE_AUTHORIZATION, TEST_AUTHORIZATION_TOKEN)
            .header(AUTHORIZATION, TEST_AUTHORIZATION_TOKEN)
            .content(objectMapper.writeValueAsString(
            ...
            .andExpect(
                jsonPath(ERRORS)
                    .value("At least one party must be selected.")
            );
    }
}
