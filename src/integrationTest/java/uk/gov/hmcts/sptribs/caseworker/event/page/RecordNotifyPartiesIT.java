...
public class RecordNotifyPartiesIT extends IntegrationTestBase {

    private static final String RECORD_NOTIFY_PARTIES_RESPONSE =
        "classpath:responses/record-notify-parties-mid-event-response.json";
    ...
    void shouldCheckRecipientInMidEvent() throws Exception {
        ...
        mockMvc.perform(post(RECORD_NOTIFY_PARTIES_MID_EVENT_URL)
            ...
            .andExpect(
                status().isOk())
            .andExpect(
                content().json(expectedResponse(RECORD_NOTIFY_PARTIES_RESPONSE))
            );
    }
}
