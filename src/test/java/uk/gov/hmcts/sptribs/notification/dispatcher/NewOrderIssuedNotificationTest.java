...
public class NewOrderIssuedNotificationTest {
    ...
    @Captor
    private ArgumentCaptor<Map<String, String>> documentTemplateVarsCaptor;
    ...
    class WhenCitizenDashboardDisabled {
        ...
        void shouldNotifySubjectOfNewOrderIssuedWithEmailWithUploadedTemplateDocument() {
            ...
            verify(notificationHelper).buildEmailNotificationRequest(eq(data.getCicCase().getEmail()), eq(true),
                documentTemplateVarsCaptor.capture(), templateVarsCaptor.capture(), eq(TemplateName.NEW_ORDER_ISSUED_EMAIL));
            assertThat(documentTemplateVarsCaptor.getValue())
        ...
        void shouldNotifyApplicantOfNewOrderIssuedWithEmail() {
            ...
            final Order order = Order.builder().uploadedFile(getCICDocumentListWithUrl("test.docx", "http://url/test.docx")).build();
            final ListValue<Order> orderListValue = new ListValue<>();
            orderListValue.setValue(order);
            data.getCicCase().setOrderList(List.of(orderListValue));
            ...
            verify(notificationHelper).buildEmailNotificationRequest(eq(data.getCicCase().getApplicantEmailAddress()), eq(true),
                documentTemplateVarsCaptor.capture(), templateVarsCaptor.capture(), eq(TemplateName.NEW_ORDER_ISSUED_EMAIL));
            assertThat(documentTemplateVarsCaptor.getValue().get(TRIBUNAL_ORDER))
                .isEqualTo(order.getUploadedFile().getFirst().getValue().getDocumentLink().getFilename());
            assertThat(templateVarsCaptor.getValue()).doesNotContainKey(DASHBOARD_KEY);
        ...
        void shouldNotifyApplicantOfNewOrderIssuedWithEmailWithNoUploadedDocument() {
            ...
            assertThat(selectedDocumentsCaptor.getValue()).isEmpty();
            assertThat(templateVarsCaptor.getValue()).doesNotContainKey(DASHBOARD_KEY);
        }
