...
public class CaseworkerContactPartiesFT extends FunctionalTestSuite {
    ...
    private static final String REQUEST_SUBMITTED =
        "classpath:request/casedata/ccd-callback-casedata-contact-parties-submitted.json";
    ...
    @Test
    public void shouldPartiesContactedSuccessfully() throws Exception {
        final Map<String, Object> caseData = caseData(REQUEST_SUBMITTED);

        final Response response = triggerCallback(caseData,
            EventConstants.CASEWORKER_CONTACT_PARTIES, SUBMITTED_URL);
        assertThat(response.getStatusCode()).isEqualTo(OK.value());
