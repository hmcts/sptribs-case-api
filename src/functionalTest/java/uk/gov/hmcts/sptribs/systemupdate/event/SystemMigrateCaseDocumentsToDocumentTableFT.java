...
public class SystemMigrateCaseDocumentsToDocumentTableFT extends FunctionalTestSuite {
    ...
    private static final String RESPONSE =
        "classpath:responses/response-system-migrate-documents-to-table-about-to-submit.json";

    @Test
    public void shouldMigrateCaseFlagsOnAboutToSubmit() throws Exception {
        final Map<String, Object> caseData = caseData(REQUEST);
        final Response response = triggerCallback(caseData, SYSTEM_MIGRATE_CASE_DOCUMENTS_TO_TABLE, ABOUT_TO_SUBMIT_URL, false);

        assertThat(response.getStatusCode()).isEqualTo(OK.value());
        assertThatJson(response.asString())
            .when(IGNORING_EXTRA_FIELDS)
            .whenIgnoringPaths("data.hyphenatedCaseRef",
                "data.cicCaseDraftOrderCICList[*].value.templateGeneratedDocument.document_filename")
            .node("data")
