...
public class CicaCaseController {
    ...
    @Operation(
        ...
        description = "Compares the submitted postcode to the postcode held in the case data for the subject. "
            + "Values match exactly, ignoring case and whitespace. Returns full case details on successful match."
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
            description = "Postcode matches successfully, returning full case details",
            content = @Content(
                mediaType = MediaType.APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = CicaCaseResponse.class)
            )),
    ...
    public ResponseEntity<CicaCaseResponse> validatePostcode(
        ...
        log.info("Successfully validated postcode and retrieved case details for CCD reference: {}", ccdReference);
        return ResponseEntity.ok(response);
    }
}
