...
@Component
public class WorkBasketInputFields implements CCDConfig<CriminalInjuriesCompensationData, State, UserRole> {
    ...
    public void configure(final ConfigBuilder<CriminalInjuriesCompensationData, State, UserRole> configBuilder) {
        configBuilder.workBasketInputFields()
            .fields(SEARCH_FIELD_LIST.stream().filter(i -> !i.getId().equals(CASE_STATE)).toList());
    }
}
