...
public class CaseEventRepository {
    ...
    private static final String SELECT_CASES_WITH_EVENT =
        "SELECT DISTINCT cd.reference FROM ccd.case_data cd "
            + "WHERE cd.id IN (SELECT DISTINCT case_data_id FROM ccd.case_event WHERE event_id = :caseEventId)";
    ...
    public List<Long> getCasesWithEvent(String caseEventId) {
        try {
            List<Long> results = namedParameterJdbcTemplate.query(
                SELECT_CASES_WITH_EVENT,
                Map.of(CASE_EVENT_ID, caseEventId),
            ...
            log.error("Failed to retrieve cases for eventId={}", caseEventId, e);
            throw new CaseEventRepositoryException(
                "Failed to retrieve cases for eventId=" + caseEventId, e);
        }
    }
