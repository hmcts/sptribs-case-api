...
import static uk.gov.hmcts.sptribs.common.servicebus.CcdMessageQueueRepository.MessageQueueCandidate;
...
public class CcdCaseEventPublisher {
    ...
    public CcdCaseEventPublisher(CcdMessageQueueRepository repository,
                                 ...
                                 Clock clock,
                                 @Value("${azure.servicebus.ccd-case-events.topic-name}") String destination,
                                 @Value("${sptribs.servicebus.message-type:CASE_EVENT}") String messageType,
                                 @Value("${sptribs.servicebus.batch-size:1000}") int batchSize,
                                 @Value("${sptribs.servicebus.published-retention-days:7}") int retentionDays) {
    ...
    public void publishPendingCaseEvents() {
        ...
        while (true) {
            List<MessageQueueCandidate> candidates = repository.findUnpublishedMessages(messageType, batchSize);
            ...
            if (!publishedIds.isEmpty()) {
                repository.markPublished(publishedIds, LocalDateTime.now(clock));
        ...
        if (retentionDays > 0) {
            ...
            int removed = repository.deletePublishedBefore(messageType, cutoff);
            if (removed > 0) {
