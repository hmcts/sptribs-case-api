...
public class PdfWatermarker {
    ...
    public Pair<byte[], Integer> shrinkAndWatermarkPdf(byte[] input, String leftText, int pageCount) throws Exception {
        try (PDDocument document = Loader.loadPDF(input)) {
            document.setAllSecurityToBeRemoved(true);
            for (PDPage page : document.getPages()) {
                scaleContent(document, page, SCALE_PERCENTAGE);
                addFooterText(document, page, leftText, "Page " + ++pageCount);
            }
            ...
            try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
                document.save(baos);
                return Pair.of(baos.toByteArray(), pageCount);
            }
